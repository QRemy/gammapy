...
def test_evaluator_no_multiresolution():
    ...
    meval.PSF_MAX_RADIUS = 3 * u.deg

    import time

    datasets.stat_sum()  # just load
    ...
    dmap.USE_MULTIRESOLUTION = False
    datasets.models = [model]
    model.spatial_model.lon_0.value = position.galactic.l.value

    for k in range(50):
        model.spatial_model.lon_0.value += 0.02
        sta_ref = datasets.stat_sum()
        npred_ref = datasets[0].npred().data.sum()
    print(time.time() - t)

    print(sta_ref, npred_ref)
